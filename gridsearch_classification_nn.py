# gridsearch_classification_nn.py
...
from utils import lightning_models, settings
...
run_dir = os.path.join("results", "Higgs", "gridsearch", f"higgs_nn_grid_{timestamp}")
...
def compute_n(P_target, L, d0, d):
    ...
    n = int(round((-b + math.sqrt(disc)) / (2 * a)))
    return max(1, n)
...
for combo in itertools.product(*values):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(X_grid)):
        ...
        print(f"\n[INFO] Grid Param: P_target={params['P_target']}, L={params['L']}, hidden_dims={hidden_dims}")
...
grid_path = os.path.join(run_dir, "gridsearch_higgs_nn.csv")
