...
# Die Klasse NN in utils/lightning_modules.py muss von "BinaryClassificationNetwork" erben.
...
batch_size = 4096
num_workers = 8
GRID_ROWS = "all"  # "all" für gesamten trainingsdatensatz, 1_000_000

name = f"{hidden_dims}"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
print(f"\n[INFO] hyperparameters: L={L}, hidden dimensions={hidden_dims}\n")



# Create unique run directory
timestamp = datetime.now().strftime("%d-%m_%H-%M")
run_dir = os.path.join("results", "Higgs", "single_model", f"higgs_nn_{name}_{timestamp}")
...
test_loss_path = os.path.join(run_dir, f"test_loss_higgs_{name}.csv")
df.to_csv(test_loss_path, index=False)
print(f"[INFO] Final test/loss saved: {test_loss_path}")
...
plt.title(
    f"NN with Dataset: {dataset_name}\n"
    f"HPs: L={L}, P={num_params}, batch size={batch_size}\n"
